...
import { Specialty, TrainerConfig, Trainee, AttendanceRecord, EvaluationDatabase, AttendanceDetail } from '../types';
...
const DataAnalytics: React.FC = () => {
  ...
  const analytics = useMemo(() => {
      ...
      const enrichedData = trainees.map(t => {
          ...
          MODULES.forEach(m => {
              const mg = (tGrades as any).modules?.[m.id];
          ...
          // Fix: cast tGrades to any to access report
          const report = (tGrades as any).report || 0;
          ...
          // Attendance
          // Fix: Access status property of AttendanceDetail object with type assertion
          const absences = Object.entries(attendance).filter(([k, v]) => k.endsWith(`-${t.id}`) && (v as AttendanceDetail).status === 'A').length;
